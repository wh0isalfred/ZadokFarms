...
Review the reference-led Shop opening documented at the end of this file. Staff access and WhatsApp confirmation are already on main; deployment configuration/provisioning remains covered by their existing runbooks.
...
## Reference-led Shop opening - 20 September 2026

Implemented on `feat/shop-reference-composition` from current main `ab85498` (staff desk already merged).

- The upper catalogue now follows the supplied reference: cream capsule desktop header, singular Zadok Farm wordmark, active Shop underline, prominent heading/supporting copy, four broad pale diagonal farm-row bands, and 5:4 produce images in a four-column desktop / two-column tablet-mobile grid. The motif stays visible on mobile; existing bottom navigation and floating basket remain.
- Availability and sorting are real accessible native selects. All published products remain visible by default (honest "All availability" label); Available now includes available/limited products. Limited/unavailable filters combine with categories and search. Sorting offers the server's farm order or A-Z; unlike-unit prices are not misleadingly compared. Empty results reset every control. Search mounts only when open and receives focus.
- Header links target existing homepage sections; no unbuilt route was added. Product images and unavailable actions no longer pretend to open nonexistent product details. Basket quantity/actions remain unchanged. The existing price/availability/follow-up notice remains immediately after the grid.
- Four versioned harvest images replace the original concept paths only in Shop cards; custom catalogue images still pass through unchanged. These are generated photographic illustrations, not actual Zadok harvest photographs. Asset paths, exact prompts and production method are in [SHOP_IMAGE_PROVENANCE.md](SHOP_IMAGE_PROVENANCE.md). Original assets remain for other existing consumers, including basket thumbnails.
- Changed areas: storefront/header/product-card presentation, globals.css, catalogue UI tests, four images and these two documents. No database, environment, dependency, request form/API, WhatsApp or staff-desk changes.
- Verification: `npm.cmd run lint`, `npm.cmd run build`, and `npm.cmd test` passed: 105 tests passed, 18 database tests explicitly skipped because no test database URL was configured for this visual increment. Focused coverage checks combined native filters, sorting without source mutation, empty reset and search focus; existing ordering/auth/basket regressions passed.
- Actual Chromium checks used the production build and nine live Supabase products at 360x800, 768x1024 and 1440x900. Screenshots visually inspected at all sizes. Verified new images decode, four decorative bands, no page overflow, categories/availability/sort/search/reset, keyboard activation/focus outline, add/increase, drawer-to-request navigation, basket recovery after refresh and long product-name wrapping. Mobile bottom navigation and fixed basket verified. No uncaught browser errors; no order submitted or production data mutated.
- Limitations: generated imagery should ultimately be replaced by approved farm photography. Browser viewport automation is not physical-device or screen-reader testing. Real database/RLS tests were not rerun for this presentation-only change. Lower homepage content and operational flows remain outside this increment.
- Independent review approved the committed visual change with no blocking defects. Its suggested search focus-outline/disclosure-state improvement was applied and rechecked.
- Next step: review this visual branch against the supplied reference before merging; retain the staff provisioning and WhatsApp deployment setup in their existing runbooks.
